...
describe('NodeVersionsService', () => {
  ...
  describe('listVersions', () => {
    it('should return versions ordered by versionNumber desc', async () => {
      ...
      const result = await service.listVersions('node-1', 'user-1')

      expect(repo.find).toHaveBeenCalledWith(
        expect.objectContaining({ where: { nodeId: 'node-1', userId: 'user-1' } }),
      )
  ...
  describe('findVersionOrFail', () => {
    it('should throw NotFoundException when version does not exist', async () => {
      ...
      await expect(service.findVersionOrFail('node-1', 'v-99', 'user-1')).rejects.toThrow()
    })

    it('should return version when found', async () => {
      repo.findOne.mockResolvedValue(makeVersion(1))

      const result = await service.findVersionOrFail('node-1', 'v-1', 'user-1')
