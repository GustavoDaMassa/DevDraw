...
describe('NodesService', () => {
  ...
  describe('getTree', () => {
    it('should query the tree with userId', async () => {
      dataSource.query.mockResolvedValue([mockNode()])

      const result = await service.getTree('user-1')

      expect(dataSource.query).toHaveBeenCalledWith(expect.stringContaining('RECURSIVE'), ['user-1'])
      expect(result).toHaveLength(1)
    })
  })

  describe('findByIdOrFail', () => {
    it('should return node when found and belongs to user', async () => {
      repo.findOne.mockResolvedValue(mockNode())

      const result = await service.findByIdOrFail('node-1', 'user-1')
    ...
    it('should throw NodeNotFoundException when node not found', async () => {
      ...
      await expect(service.findByIdOrFail('node-1', 'user-1')).rejects.toBeInstanceOf(
        NodeNotFoundException,
      )
    })

    it('should decrypt content when node has encrypted content', async () => {
      ...
      expect(cryptoService.decrypt).toHaveBeenCalledWith(encrypted, 'user-1')
      expect(result.content).toEqual(Buffer.from('{"type":"draw"}'))
    })
  })

  describe('create', () => {
    it('should create and save a FOLDER node', async () => {
      const node = mockNode({ type: NodeType.FOLDER })
      repo.create.mockReturnValue(node)
      repo.save.mockResolvedValue(node)

      const result = await service.create('user-1', { name: 'Backend', type: NodeType.FOLDER })

      expect(repo.create).toHaveBeenCalledWith(
        expect.objectContaining({ name: 'Backend', type: NodeType.FOLDER, userId: 'user-1' }),
      )
  ...
  describe('update', () => {
    it('should update name and save', async () => {
      ...
      const result = await service.update('node-1', 'user-1', { name: 'Renamed' })
    ...
    it('should throw NodeNotFoundException when node does not exist', async () => {
      ...
      await expect(service.update('node-1', 'user-1', { name: 'X' })).rejects.toBeInstanceOf(
        NodeNotFoundException,
      )
    })
  })
  ...
  describe('saveContent', () => {
    it('should encrypt content, save node, and create a version', async () => {
      const node = mockNode()
      repo.findOne.mockResolvedValue(node)
      ...
      await service.saveContent('node-1', 'user-1', '{"type":"draw"}')

      expect(cryptoService.encrypt).toHaveBeenCalledWith('{"type":"draw"}', 'user-1')
    ...
    it('should throw NodeNotFoundException when node does not exist', async () => {
      ...
      await expect(service.saveContent('node-1', 'user-1', '{}')).rejects.toBeInstanceOf(
        NodeNotFoundException,
      )
    })
  })
  ...
  describe('softDelete', () => {
    it('should set deletedAt and save', async () => {
      ...
      await service.softDelete('node-1', 'user-1')
    ...
    it('should throw NodeNotFoundException when node does not exist', async () => {
      ...
      await expect(service.softDelete('node-1', 'user-1')).rejects.toBeInstanceOf(NodeNotFoundException)
    })
  })
})
